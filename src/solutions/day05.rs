...
pub fn part1() -> String {
    ...
    let mut piles: Vec<Vec<char>> = piles.into_iter().map(|pile: Vec<char>| pile.into_iter().rev().collect()).collect();
    ...
    piles.into_iter().map(|pile: Vec<char>| pile.last().unwrap().to_string()).collect::<Vec<String>>().join("")
}
